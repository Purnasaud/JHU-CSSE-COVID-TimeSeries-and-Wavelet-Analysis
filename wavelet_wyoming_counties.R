...
# Precompute global maximum power across all counties
global_max <- 0
wavelet_list <- list()
...
for(cty in counties) {
  ...
  current_max <- max(counties_wt$Power, na.rm = TRUE)
  global_max <- max(global_max, current_max)
}
message("Global maximum wavelet power: ", global_max)
...
for(cty in counties) {
  ...
  wt.image(
    ...
    legend.params = list(
      ...
      lab = NULL,
      lab.line = 2.5,
      lab.break = paste0("0 - ", round(global_max, 2))
    ),
