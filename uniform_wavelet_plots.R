...
input_file <- "D:/WY-COVID Data/Wyoming_Covid-Analysis/new_Smoothened_cases.csv"
out_dir    <- "D:/WY-COVID Data/Wyoming_Covid-Analysis/R_wavelet_plots/Final_Wavelet_Plots"
...
for(cty in counties) {
  ...
  cd <- covid_data %>%
    filter(County == cty) %>%
    select(Date, cases = smoothed_Cases_ratio) %>%
    filter(!is.na(cases)) %>%
    arrange(Date) %>%
    mutate(
      t            = as.numeric(as.Date(Date) - min(as.Date(Date))) + 1,
      detrended    = resid(lm(cases ~ poly(t,2), data = .)),
